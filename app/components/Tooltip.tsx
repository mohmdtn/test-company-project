...
const Tooltip = () => {
  ...
  return (
    <section className="w-full md:w-[21.625rem] text-white rounded-[1.25rem] bg-[#231F2F] shadow-md shadow-gray-500 px-2 py-3 h-min">
      ...
      {/* Less Content */}
      <p className={`text-[#CBCBCB] text-sm my-3 ps-5 pe-8 ${!isShowMore ? "max-h-[100vh] opacity-100" : "max-h-0 opacity-0"}`}>
        Betterment . Gain . rising . improvement. rising . improvement.
      </p>

      {/* More Content */}
      <div className={`flex flex-col gap-4 duration-700 overflow-hidden ${isShowMore ? "max-h-[100vh] opacity-100" : "max-h-0 opacity-0"}`}>
        {tootipInfo.map((info) => {
          return (
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row justify-between mt-4 px-5">
        <ButtonCustom
          ...
          classname={`bg-inherit rounded-full text-[#6157C8] flex px-4 items-center normal-case border border-[#6157C8] duration-500 hover:border-[#9a4dff] ${isShowMore ? "opacity-0" : "opacity-100"}`}
        />
        <div className="flex gap-1 items-center">
          <MdOutlineSignalCellularAlt size={20} className="text-[#6157C8]" />
